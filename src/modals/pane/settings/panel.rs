...
use crate::components::primitives::label::{body, label_text, title};
...
use data::config::panel::ladder;
use data::config::panel::timeandsales::{StackedBar, StackedBarRatio};
use data::state::pane::{LadderConfig, TimeAndSalesConfig, VisualConfig};
            ...
            move |value| {
                let stacked_bar = cfg.stacked_bar.map(|sb| {
                    let is_compact = matches!(
                        sb,
                        data::config::panel::timeandsales::StackedBar::Compact(_)
                    );
                    let ratio = sb.ratio();
                    (is_compact, ratio)
                });
    ...
    let content = split_column![
        ...
        row![space::horizontal(), sync_all_button(pane, VisualConfig::TimeAndSales(TimeAndSalesConfig {
            ...
            stacked_bar: cfg.stacked_bar.map(|sb| {
                let is_compact = matches!(sb, data::config::panel::timeandsales::StackedBar::Compact(_));
                (is_compact, sb.ratio())
            }),
        }))],
