...
impl DataFeedsModal {
    pub fn view(&self) -> Element<'_, DataFeedsMessage> {
        let title_bar = container(component::primitives::heading("Manage Connections"))
    ...
    fn view_left_panel(&self) -> Element<'_, DataFeedsMessage> {
        ...
        if feeds.total_count() == 0 && !self.is_creating {
            feed_list = feed_list.push(
                container(component::primitives::body("No connections"))
        ...
        // "+" and "-" buttons
        let add_button = button(component::primitives::title("+").align_x(Alignment::Center))
            .width(28)
            .height(28)
            .on_press(DataFeedsMessage::ToggleAddPopup);

        let remove_button: Option<Element<'_, DataFeedsMessage>> = self.selected_feed.map(|id| {
            button(component::primitives::title("\u{2212}").align_x(Alignment::Center))
                .width(28)
                .height(28)
        ...
        column![
            mouse_area(scrollable(feed_list.padding([tokens::spacing::XS, 0.0])).height(Length::Fill),)
    ...
    fn view_feed_item<'a>(
        ...
        let indicator: Element<'a, DataFeedsMessage> = if feed.is_historical() {
            ...
            component::display::status_dot(status_color)
        };

        let info = column![
            component::primitives::label_text(&feed.name),
            component::primitives::tiny(feed.provider.display_name()),
        ]
        .spacing(tokens::spacing::XXXS);

        let item_content = row![indicator, info].spacing(tokens::spacing::MD).align_y(Alignment::Center);
    ...
    fn view_right_panel(&self) -> Element<'_, DataFeedsMessage> {
        ...
        match self.selected_feed {
            None => container(component::primitives::label_text(
            ...
            Some(id) => {
                if let Some(feed) = feeds.get(id) {
                    ...
                    container(component::primitives::label_text("Feed not found"))
                        .width(Length::Fill)
                        .height(Length::Fill)
                        .align_x(Alignment::Center)
                        .align_y(Alignment::Center)
    ...
    fn view_edit_form<'a>(&'a self, feed: &'a DataFeed) -> Element<'a, DataFeedsMessage> {
        ...
        let type_col: Element<'_, DataFeedsMessage> = if self.is_creating {
            column![
                component::primitives::body("Type"),
                pick_list(
                    FeedProvider::ALL,
        ...
        let footer = container(
            row![
                ...
                button(component::primitives::label_text("Cancel"))
                    .on_press(DataFeedsMessage::CancelEdit)
                    .padding([tokens::spacing::XS, tokens::spacing::LG])
                    .style(style::button::secondary),
                button(component::primitives::label_text("Save"))
    ...
    fn view_databento_fields(&self) -> Element<'_, DataFeedsMessage> {
        ...
        column![
            component::primitives::title("Databento Settings"),
            api_key_field,
            cache_toggle,
            cache_days,
        ]
    ...
    fn view_rithmic_fields(&self) -> Element<'_, DataFeedsMessage> {
        ...
        let env_system_row = row![
            column![
                component::primitives::body("Environment"),
            ...
            .width(Length::FillPortion(1)),
            column![
                component::primitives::body("System Name"),
                pick_list(
                    system_name_options,
                    selected_system,
        ...
        let user_id = column![
            component::primitives::body("User ID"),
...
fn section_header(label: &str) -> Element<'_, DataFeedsMessage> {
    container(component::primitives::small(label))
        .padding(
