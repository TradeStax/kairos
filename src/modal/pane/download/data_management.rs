...
use super::super::calendar::{DateRangeCalendar, CalendarMessage};
...
impl DataManagementPanel {
    ...
    pub fn with_ticker(mut self, ticker: FuturesTicker) -> Self {
        ...
        if let Some(idx) = super::super::FUTURES_PRODUCTS
            .iter()
    ...
    pub fn update(&mut self, message: DataManagementMessage) -> Option<Action> {
        match message {
            ...
            DataManagementMessage::ConfirmDownload => {
                ...
                let num_days =
                    (self.calendar.end_date - self.calendar.start_date).num_days().max(0) + 1;
    ...
    fn trigger_estimation(&mut self, date_range: Option<DateRange>) -> Option<Action> {
        ...
        let ticker = DownloadConfig::ticker_from_idx(self.selected_ticker_idx);
        let schema = DownloadConfig::schema_from_idx(self.selected_schema_idx);
        let range = date_range.unwrap_or_else(|| {
            DateRange::new(self.calendar.start_date, self.calendar.end_date)
        });
    ...
    pub fn view(&self) -> Element<'_, DataManagementMessage> {
        let ticker_section =
            views::ticker_dropdown(self.selected_ticker_idx, DataManagementMessage::TickerSelected);

        let schema_section =
            views::schema_dropdown(self.selected_schema_idx, DataManagementMessage::SchemaSelected);
        ...
        let cache_summary = views::cache_status_display(
            &self.download_progress,
        ...
        let base_content = content_items.into_iter().fold(
            column![].spacing(tokens::spacing::LG).align_x(Alignment::Start),
