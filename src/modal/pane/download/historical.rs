...
use super::views;
use super::super::calendar::{CalendarMessage, DateRangeCalendar};
...
impl HistoricalDownloadModal {
    ...
    pub fn auto_name(&self) -> String {
        let (sym, _) = super::super::FUTURES_PRODUCTS[self.selected_ticker_idx];
        let short_sym = sym.split('.').next().unwrap_or(sym);
        let (_, schema_name, _) = super::super::SCHEMAS[self.selected_schema_idx];
