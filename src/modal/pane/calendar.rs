...
impl DateRangeCalendar {
    ...
    pub fn update(&mut self, message: CalendarMessage) -> bool {
        match message {
            CalendarMessage::PrevMonth => {
                let prev =
                    self.viewing_month - chrono::Months::new(1);
                self.viewing_month = NaiveDate::from_ymd_opt(
                    prev.year(),
                    prev.month(),
                    1,
                )
    ...
    fn build_grid<M: Clone + 'static>(
        ...
        let calendar_start =
            first_day - chrono::Duration::days(days_until_monday);
        ...
        for week in 0..6 {
            ...
            for day in 0..5 {
                let date = calendar_start
                    + chrono::Duration::days(week * 7 + day);
                ...
                let is_current_month = date.month() == month.month()
                ...
                let day_button = button(day_text)
                    ...
                    .on_press_maybe(if is_cached {
                        ...
                        Some(map_msg(CalendarMessage::DayClicked(
                            date,
                        )))
                    });
