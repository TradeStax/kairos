...
use iced::{
    ...
    alignment,
    padding,
    widget::{button, center, column, container, row, rule, text, text_input},
};
...
fn filtered_rows<'a>(
    ...
    rows.sort_by(|(_, ra), (_, rb)| {
        (ra.bucket, ra.pos, ra.len)
            .cmp(&(rb.bucket, rb.pos, rb.len))
    });
    ...
    FSelect: 'static + Copy + Fn(RowSelection) -> M,
{
    ...
    for row_ref in rows {
        ...
        let info_opt: Option<FuturesTickerInfo> =
            tickers_info.get(&row_ref.ticker).copied();
