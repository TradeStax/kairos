...
use data::state::pane_config::{CandleColorField, CandleStyle, KlineConfig, VisualConfig};
...
use iced::{
    ...
    widget::{
        button, column, container, mouse_area, opaque, pick_list, row, slider,
        space, stack, text,
    },
};
        ...
        KlineChartKind::Candles => {
            ...
            let reset_all_btn = {
                ...
                button(text("Reset all").size(12))
                    .on_press_maybe(has_any_custom.then(|| {
            ...
            if let Some(field) = editing {
                ...
                let display_color =
                    current_color.unwrap_or(default_for_field(field));
                ...
                let picker = color_picker(
                    ...
                    move |new_hsva| {
                        ...
                        let mut new_style =
                            cfg_for_picker.candle_style.clone();
                ...
                let popup = container(
                    column![
                        text(label).size(tokens::text::LABEL),
                        picker,
                    ]
                    .spacing(tokens::spacing::SM),
                )
                ...
                cfg_view_container(
                    320,
                    stack![
                        mouse_area(compact_col).on_press(dismiss),
                        opaque(popup),
                    ],
                )
            } else {
...
fn default_for_field(field: CandleColorField) -> Color {
    match field {
        ...
        CandleColorField::BullBorder | CandleColorField::BearBorder => {
            Color::TRANSPARENT
        }
    }
}
