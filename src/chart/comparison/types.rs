...
pub(super) fn ticks(min: f32, max: f32, target: usize) -> (Vec<f32>, f32) {
...
pub(super) fn format_pct(val: f32, step: f32, show_decimals: bool) -> String {
    if show_decimals {
        if step >= 1.0 {
            format!("{:+.1}%", val)
        } else if step >= 0.1 {
...
pub(super) fn format_time_label(ts_ms: u64, step_ms: u64) -> String {
