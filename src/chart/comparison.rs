...
pub struct ComparisonChart {
    ...
    /// Series editor state
    series_editor: series_editor::TickerSeriesEditor,
...
impl ComparisonChart {
    ...
    pub fn series_editor(&self) -> &series_editor::TickerSeriesEditor {
        &self.series_editor
    }
...
fn ticker_info_to_old_format(info: FuturesTickerInfo) -> TickerInfo {
    TickerInfo::new(info.ticker, info.tick_size, info.min_qty, info.contract_size)
}

fn old_format_to_ticker_info(info: &TickerInfo) -> FuturesTickerInfo {
...
pub mod series_editor {
    ...
    impl TickerSeriesEditor {
        ...
        pub fn view<'a>(&'a self, series: &'a [Series]) -> Element<'a, Message> {
            ...
            for s in series {
                let applied = s.color;
                let futures_info = super::old_format_to_ticker_info(&s.ticker_info);
