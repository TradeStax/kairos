//! Plot rendering for indicators
//!
//! This module provides the canvas rendering infrastructure for
...
impl<'a, Y> AnySeries<'a, Y> {
    pub fn for_basis(basis: ChartBasis, data: &'a BTreeMap<u64, Y>) -> Self {
        match basis {
            ChartBasis::Tick(_) => Self::Reversed(ReversedBTreeSeries::new(data)),
            ChartBasis::Time(_) => Self::Forward(data),
        }
    }
}
