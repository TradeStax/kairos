...
use iced::widget::canvas::{
    self, Frame, Geometry, LineDash, Path, Stroke, Text as CanvasText,
};
use iced::{
    mouse, Color, Element, Length, Point, Rectangle, Renderer, Size, Theme,
};
...
impl<Message> canvas::Program<Message>
    for VolumeTrackbarProgram<'_, Message>
{
                ...
                if let Some(pos) = cursor.position_in(bounds) {
                    if pos.y >= TOOLTIP_RESERVE {
                        *state = Interaction::Dragging;
                        let progress =
                            (pos.x / bounds.width).clamp(0.0, 1.0);
                        return Some(canvas::Action::publish(
                            (self.on_seek)(progress),
                        ));
                    }
                }
            }
    ...
    fn draw(
        ...
        frame.stroke(
            ...
            Stroke::default()
                .with_width(1.0)
                .with_color(Color { a: 0.2, ..bg_strong }),
        );
        ...
        if max_vol > 0.0 {
            for (i, bucket) in self.buckets.iter().enumerate() {
                ...
                let is_partial =
                    x < playhead_x && x + bar_w > playhead_x;
        ...
        if let Some(pos) = cursor.position_in(bounds) {
            ...
            let dash_line = Path::line(
                Point::new(hover_x, top),
                Point::new(hover_x, top + h),
            );
            frame.stroke(
                &dash_line,
                Stroke {
            ...
            if let Some(range) = self.time_range {
                ...
                let hover_ms = start_ms
                    + ((end_ms - start_ms) as f32 * hover_progress)
                        as u64;

                let label = self
                    .timezone
                    .format_replay_tooltip(hover_ms as i64);
                if !label.is_empty() {
