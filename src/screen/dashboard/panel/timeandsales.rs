...
impl TimeAndSales {
    ...
    /// Update from replay data - converts domain Trades to TradeEntries for display
    pub fn update_from_replay(&mut self, trades: &[Trade]) {
        ...
        for trade in trades {
            // Convert domain Trade to TradeEntry for display
            let trade_entry = convert_trade_to_entry(trade);

            // Apply size filter
            if trade_entry.quantity >= size_filter {
                self.max_filtered_qty = self.max_filtered_qty.max(trade_entry.quantity);
            }
