...
pub struct Ladder {
    ...
    raw_price_spread: Option<ExPrice>,
}
...
impl Ladder {
    ...
    pub fn update_from_replay(&mut self, depth: &DepthSnapshot, trades: &[Trade]) {
    ...
    fn trade_qty_at(&self, price: ExPrice) -> (f32, f32) {
    ...
    fn grouped_asks(&self) -> &BTreeMap<ExPrice, f32> {
        &self.orderbook[Side::Ask.idx()].orders
    }

    fn grouped_bids(&self) -> &BTreeMap<ExPrice, f32> {
    ...
    fn best_price(&self, side: Side) -> Option<ExPrice> {
...
impl PriceGrid {
    ...
    fn index_to_price(&self, idx: i32) -> Option<ExPrice> {
        if idx == 0 {
            return None;
        }
        if idx > 0 {
...
impl TradeStore {
    ...
    fn price_range(&self) -> Option<(ExPrice, ExPrice)> {
    ...
    fn maybe_cleanup(&mut self, now_ms: u64, retention_ms: u64, step: PriceStep) -> bool {
...
impl ChaseTracker {
    ...
    fn segment(&self) -> Option<(i64, i64, f32)> {
        match self.state {
            ChaseState::Chasing {
                start_units,
                end_units,
