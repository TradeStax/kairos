    ...
    S: SeriesLike,
{
    pub(super) fn compute_scene(&self, layout: Layout<'_>, cursor: mouse::Cursor) -> Option<Scene> {
        ...
        let (all_ticks, step) = super::super::ticks(min_pct, max_pct, total_ticks);
        ...
        let labels: Vec<String> = ticks
            .iter()
            .map(|t| super::super::format_pct(*t, step, false))
            .collect();
        ...
        if should_draw_crosshair && let Some(ci) = cursor_info {
            ...
            let pct_str = super::super::format_pct(ci.y_pct, step, true);
