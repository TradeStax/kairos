...
pub struct FootprintStudy {
    ...
    candle_levels: Vec<BTreeMap<i64, (f32, f32)>>,
    /// Per-candle grouping quantum (price units per row)
    candle_quantums: Vec<i64>,
}
...
impl FootprintStudy {
    ...
    fn mode(&self) -> FootprintRenderMode {
    ...
    fn data_type(&self) -> FootprintDataType {
    ...
    fn scaling(&self) -> FootprintScaling {
    ...
    fn marker_alignment(&self) -> FootprintCandlePosition {
    ...
    fn outside_bar_style(&self) -> OutsideBarStyle {
    ...
    fn bg_color_mode(&self) -> BackgroundColorMode {
        match self
            .config
    ...
    fn build_output(
        ...
        let color_or_none = |key: &str| -> Option<SerializableColor> {
            self.config.get(key).and_then(|v| match v {
                ParameterValue::Color(c) if c.a > 0.0 => {
                    Some(*c)
                }
                _ => None,
            })
        ...
        StudyOutput::Footprint(FootprintData {
            ...
            candles: fp_candles,
            bar_marker_width: self
                .config
                .get_float("bar_marker_width", 0.25)
                as f32,
            outside_bar_style: self.outside_bar_style(),
            show_outside_border: self
                .config
                .get_bool("show_outside_border", false),
            max_bars_to_show: self
                .config
                .get_int("max_bars_to_show", 200)
                as usize,
            bg_color_mode: self.bg_color_mode(),
            bg_max_alpha: self.config.get_float("bg_max_alpha", 0.6)
                as f32,
            bg_buy_color,
            bg_sell_color,
            show_grid_lines: self
                .config
                .get_bool("show_grid_lines", true),
            font_size: self.config.get_float("font_size", 11.0) as f32,
            text_format: self.text_format(),
            dynamic_text_size: self
                .config
                .get_bool("dynamic_text_size", true),
            show_zero_values: self
                .config
                .get_bool("show_zero_values", false),
            text_color,
            grouping_mode,
        })
    }
    ...
    /// index.
    fn aggregate_trades_for_candle(
        &mut self,
        candle_idx: usize,
        trades: &[Trade],
        group_quantum: i64,
...
impl Study for FootprintStudy {
    ...
    fn compute(&mut self, input: &StudyInput) -> Result<(), StudyError> {
        ...
        self.candle_levels.clear();
        self.candle_levels
            .resize_with(candle_count, BTreeMap::new);
        self.candle_quantums.clear();
        ...
        for (idx, candle) in input.candles.iter().enumerate() {
            ...
            self.aggregate_trades_for_candle(
                idx,
                &trades[start_trade..end_trade],
                quantum,
            );
        }
    ...
    fn append_trades(
        ...
        if self.candle_levels.len() <= last_idx {
            self.candle_levels
                .resize_with(last_idx + 1, BTreeMap::new);
        }
...
mod tests {
    ...
    fn test_footprint_compute() {
        ...
        let candles = vec![make_candle(
            1000, 100.0, 102.0, 99.0, 101.0, 50.0, 30.0,
        )];
        let trades = vec![
    ...
    fn test_footprint_append_trades() {
        let mut study = FootprintStudy::new();
        let candles = vec![make_candle(
            1000, 100.0, 101.0, 99.0, 100.0, 10.0, 10.0,
        )];
        let trades = vec![
        ...
        match study.output() {
            StudyOutput::Footprint(data) => {
                ...
                let level_prices: Vec<i64> = data.candles[0]
                    .levels
                    .iter()
                    .map(|l| l.price)
                    .collect();
    ...
    fn test_tick_grouping_manual() {
        ...
        study
            .set_parameter(
                "group_mode",
                ParameterValue::Choice("Fixed".to_string()),
            )
            .unwrap();

        let candles = vec![make_candle(
            1000, 100.0, 104.0, 99.0, 103.0, 50.0, 50.0,
        )];
    ...
    fn test_tick_grouping_automatic() {
        ...
        let candles = vec![make_candle(
            1000, 100.0, 120.0, 99.0, 115.0, 100.0, 100.0,
        )];
    ...
    fn test_new_parameters_accepted() {
        ...
        let test_params: Vec<(&str, ParameterValue)> = vec![
            (
                "data_type",
                ParameterValue::Choice("Delta".to_string()),
            ),
            ...
            (
                "scaling",
                ParameterValue::Choice("Linear".to_string()),
            ),
            (
                "bg_color_mode",
                ParameterValue::Choice(
                    "Delta Intensity".to_string(),
                ),
            ),
            ...
            (
                "text_format",
                ParameterValue::Choice("K".to_string()),
            ),
    ...
    fn test_build_output_fields() {
        ...
        let trades =
            vec![make_trade(1000, 100.0, 20.0, Side::Buy)];
        let input = StudyInput {
            candles: &candles,
            trades: Some(&trades),
            basis: ChartBasis::Time(Timeframe::M1),
    ...
    fn test_max_bars_does_not_affect_compute() {
        ...
        let candles = vec![
            ...
            make_candle(
                61000, 101.0, 103.0, 100.0, 102.0, 40.0, 20.0,
            ),
            make_candle(
                121000, 102.0, 104.0, 101.0, 103.0, 60.0, 10.0,
            ),
        ];
        let trades = vec![
            make_trade(1000, 100.0, 20.0, Side::Buy),
