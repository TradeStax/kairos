...
pub fn build_profile_from_candles(
    ...
    for c in candles {
        let low_units = (c.low.round_to_tick(tick_size).units() / step) * step;
        let high_units = ((c.high.round_to_tick(tick_size).units() + step - 1) / step) * step;

        if step <= 0 || high_units < low_units {
            continue;
        }

        let body_low_units = (c.open.min(c.close).round_to_tick(tick_size).units() / step) * step;
        let body_high_units =
            ((c.open.max(c.close).round_to_tick(tick_size).units() + step - 1) / step) * step;
        ...
        if body_count <= 1.0 && lower_wick_count == 0.0 && upper_wick_count == 0.0 {
...
pub fn build_profile_from_trades(
    ...
    let mut volume_map: HashMap<i64, (f64, f64)> = HashMap::with_capacity(trades.len() / 4 + 16);

    for t in trades {
        let price_units = if group_quantum > 0 {
            (t.price.round_to_tick(tick_size).units() / group_quantum) * group_quantum
        } else {
...
fn percentile_value(volumes: &[f32], pct: f32) -> f32 {
    ...
    let idx = ((pct * (working.len() - 1) as f32) as usize).min(working.len() - 1);
    working.select_nth_unstable_by(idx, |a, b| a.partial_cmp(b).unwrap());
    working[idx]
}
...
fn prominence(idx: usize, volumes: &[f32]) -> f32 {
    ...
    let right = if idx + 1 < volumes.len() {
        volumes[idx + 1]
    } else {
        vol
    };
...
mod tests {
    ...
    fn test_build_profile_from_candles() {
        ...
        let levels = build_profile_from_candles(&candles, tick_size, tick_size.units());
    ...
    fn test_detect_nodes_known_peaks() {
        ...
        // HVN at 100 and 103
        assert!(hvn.len() >= 2);
        let hvn_prices: Vec<f64> = hvn.iter().map(|n| n.price).collect();
        assert!(hvn_prices.contains(&100.0));
        assert!(hvn_prices.contains(&103.0));

        // LVN at 102 (8.0 <= 0.15 * 100 = 15.0)
        assert!(!lvn.is_empty());
        let lvn_prices: Vec<f64> = lvn.iter().map(|n| n.price).collect();
    ...
    fn test_detect_zones_two_clusters() {
        ...
        assert_eq!(hvn_zones.len(), 2, "Should find two HVN zones");
        // First zone covers 98-100
        assert_eq!(hvn_zones[0].0, Price::from_f64(98.0).units());
        assert_eq!(hvn_zones[0].1, Price::from_f64(100.0).units());
        // Second zone covers 103-104
        assert_eq!(hvn_zones[1].0, Price::from_f64(103.0).units());
        assert_eq!(hvn_zones[1].1, Price::from_f64(104.0).units());
    }
