...
//!
//! Computes session VWAP with Welford online variance. Enters on price
//! deviation beyond `deviation_bands` standard deviations. Exits at
//! VWAP cross (or opposite band).
...
/// Slope of the last `n` candle closes (linear regression slope as
/// pct change).
...
fn make_params() -> Vec<ParameterDef> {
    vec![
        ParameterDef {
            ...
            description: "Number of standard deviations from \
                          VWAP for entry."
                .into(),
...
impl Strategy for VwapReversionStrategy {
    ...
    fn on_candle(
        ...
        if ctx.primary_position().is_none() && self.trades_taken < self.max_trades() {
            ...
            if close <= lower {
                ...
                let sl = entry - tick_size.add_steps(self.fixed_stop_ticks(), tick_size);
                ...
                return vec![OrderRequest::SubmitBracket(BracketOrder {
                    ...
                    stop_loss: sl,
                    take_profit: if self.exit_at_vwap() {
                        Some(tp_price)
                    } else {
                        None
                    },
