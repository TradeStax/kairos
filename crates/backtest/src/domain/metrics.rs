...
impl PerformanceMetrics {
    ...
    pub fn compute(
        ...
        let win_rate =
            if total_trades > 0 { winning_trades as f64 / total_trades as f64 } else { 0.0 };

        let wins: Vec<f64> =
            trades.iter().filter(|t| t.pnl_net_usd > 0.0).map(|t| t.pnl_net_usd).collect();
        let losses: Vec<f64> =
            trades.iter().filter(|t| t.pnl_net_usd < 0.0).map(|t| t.pnl_net_usd).collect();
        ...
        let best_trade_usd =
            trades.iter().map(|t| t.pnl_net_usd).fold(f64::NEG_INFINITY, f64::max);
        let worst_trade_usd =
            trades.iter().map(|t| t.pnl_net_usd).fold(f64::INFINITY, f64::min);
        ...
        let annualized_return = if trading_days > 0 {
            total_return_pct / 100.0 * (252.0 / trading_days as f64)
        } else {
            0.0
        };
        let calmar_ratio = if max_drawdown_pct.abs() < 1e-10 {
            ...
            annualized_return / max_drawdown_pct.abs() * 100.0
        };
...
fn compute_sortino(daily_returns: &[f64], risk_free_daily: f64) -> f64 {
    ...
    let downside: Vec<f64> = daily_returns
        .iter()
        .map(|r| if *r < risk_free_daily { r - risk_free_daily } else { 0.0 })
        .collect();
    let downside_std = std_dev(&downside);
    if downside_std == 0.0 {
        return 0.0;
    }
    mean(&excess) / downside_std * 252_f64.sqrt()
}
...
fn compute_max_drawdown(curve: &EquityCurve, initial_capital: f64) -> (f64, f64) {
    ...
    let max_dd_pct =
        if max_dd_peak > 0.0 { max_dd_usd / max_dd_peak * 100.0 } else { 0.0 };
