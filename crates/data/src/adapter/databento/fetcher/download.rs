...
impl DatabentoAdapter {
    /// Fetch a single day to the .dbn.zst cache, then decode+write to unified cache
    pub(crate) async fn fetch_to_unified_cache(
        ...
        // Check if already in unified cache
    ...
    /// Load a day's trades from unified cache
    ...
    /// Load a day's depth snapshots from unified cache
    ...
    /// Fetch a range, caching each day
