        ...
        match self.config.basis {
            ChartBasis::Time(tf) => {
                ...
                if let Some(last_candle) = self.data.candles.last_mut()
                    && last_candle.time.to_millis() == bucket_time
                {
                    ...
                    last_candle.buy_volume = Volume(last_candle.buy_volume.0 + buy_vol.0);
                    last_candle.sell_volume = Volume(last_candle.sell_volume.0 + sell_vol.0);
                    return;
                }
                // New candle period
            ...
            ChartBasis::Tick(count) => {
                ...
                let trades_in_current = num_trades.saturating_sub(completed_candles * count);
