...
impl DateRange {
    ...
    /// Create a date range for the last N days (ending yesterday)
    pub fn last_n_days(n: i64) -> Self {
        let end = Self::today_et() - chrono::Duration::days(1);
        let start = end - chrono::Duration::days(n - 1);
        Self { start, end }
    }

    /// Previous trading week (Monday–Friday before the current week)
    pub fn last_week() -> Self {
        use chrono::Weekday;
        let today = Self::today_et();
        // Days since last Monday (0 = Monday, 6 = Sunday)
...
impl Side {
    ...
    /// Get index (0 for Bid/Buy, 1 for Ask/Sell)
