...
pub(super) fn parse_color_name(
    name: &str,
) -> data::SerializableColor {
    match name.to_lowercase().as_str() {
        ...
        "gray" | "grey" => {
            data::SerializableColor::new(0.6, 0.6, 0.6, 1.0)
        }
        "cyan" => data::SerializableColor::from_rgb8(0, 200, 220),
        "magenta" | "pink" => {
            data::SerializableColor::from_rgb8(255, 100, 180)
        }
...
pub(super) fn parse_line_style(
    name: &str,
) -> data::LineStyle {
    match name.to_lowercase().as_str() {
        "solid" => data::LineStyle::Solid,
        "dashed" => data::LineStyle::Dashed,
        "dotted" => data::LineStyle::Dotted,
        _ => data::LineStyle::Dashed,
    }
}
...
pub fn execute_tool(
    ...
    match name {
        ...
        "get_volume_profile" => {
            trades::exec_get_volume_profile(snap, &args)
        }
        "get_delta_profile" => {
            trades::exec_get_delta_profile(snap, &args)
        }
        "get_aggregated_trades" => {
            trades::exec_get_aggregated_trades(snap, &args)
        }
        // Studies
        "get_study_values" => {
        ...
        "get_footprint" => {
            studies::exec_get_footprint(snap, &args)
        }
        "get_profile_data" => {
        ...
        // Drawing actions
        "add_horizontal_line" | "add_text_annotation"
        | "add_price_level" | "add_rectangle" | "add_arrow"
        | "remove_drawing" => {
            drawings::execute_drawing_tool(name, &args, ctx)
        }
        _ => ToolExecResult {
            content_json: json!({
