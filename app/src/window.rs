    ...
    M: MaybeSend + 'static,
{
    ...
    Task::batch(window_spec_tasks)
        .collect()
        .map(move |results| {
            // NOTE: iced::window::position() may return None on Wayland or if the window
            // has never been moved. We do NOT drop the window in that case — we keep it
            // in the map with x/y = None so the WindowSpec is still saved with valid size.
            // WindowSpec.x/y are Option<i32>, and SavedState::window() falls back to
            // Position::Centered when coordinates are missing.
            let specs: HashMap<window::Id, WindowSpec> = results
                .into_iter()
                .map(|(id, (pos, size))| {
                    let spec = WindowSpec {
                        x: pos.map(|p| p.x as i32),
                        y: pos.map(|p| p.y as i32),
