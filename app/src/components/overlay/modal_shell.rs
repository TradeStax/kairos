use iced::widget::{center, column, container, mouse_area, opaque, scrollable, stack, text};
...
impl<'a, Message: Clone + 'a> ModalShell<'a, Message> {
    ...
    pub fn view(self, base: impl Into<Element<'a, Message>>) -> Element<'a, Message> {
        ...
        if let Some(title) = self.title {
            inner = inner.push(text(title).size(tokens::text::HEADING).width(Length::Fill));
        }

        // Scrollable body
        let body_scrollable = scrollable(self.body).style(style::scroll_bar);
        ...
        // -- Container --------------------------------------------------------
        let mut modal_container = container(inner).padding(self.padding).style(style_fn);
