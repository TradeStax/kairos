...
use iced::{
    ...
    widget::{
        button, column, container, pick_list, row, space, text,
    },
};
    ...
    pub(super) fn vbp_param_widget<'a>(
        ...
        match &param.kind {
            ...
            ParameterKind::Color => {
                ...
                let is_editing = self.vbp.as_ref().is_some_and(|v| {
                    v.editing_color_key.as_deref() == Some(param.key.as_str())
                });
                ...
                if is_editing {
                    let hsva = self
                        .vbp.as_ref().and_then(|v| v.editing_color_hsva)
                        .unwrap_or_else(|| {
