...
use iced::{
    ...
    widget::{
        button, column, container, row, rule, scrollable, space, text,
    },
};
...
pub(super) fn category_badge(
    ...
    let (label, kind) = match category {
        backtest::StrategyCategory::BreakoutMomentum => {
            ("Breakout", BadgeKind::Warning)
        }
        backtest::StrategyCategory::MeanReversion => {
            ("Mean Rev.", BadgeKind::Info)
        }
        backtest::StrategyCategory::TrendFollowing => {
            ("Trend", BadgeKind::Success)
        }
        backtest::StrategyCategory::OrderFlow => {
            ("Order Flow", BadgeKind::Default)
        }
        backtest::StrategyCategory::Custom => {
            ("Custom", BadgeKind::Default)
        }
    };
    badge(label, kind)
}
...
impl BacktestLaunchModal {
    ...
    pub fn view(&self) -> Element<'_, Message> {
        let header = ModalHeaderBuilder::new("Backtest Strategy")
    ...
    pub(super) fn view_search_and_filter(
        ...
        let filter_items: Vec<(String, Message)> = CategoryFilter::ALL
            .iter()
            .map(|cat| {
                (cat.to_string(), Message::CategorySelected(*cat))
            })
            .collect();
    ...
    pub(super) fn view_left_panel(&self) -> Element<'_, Message> {
        ...
        for (id, strategy) in &filtered {
            let is_selected =
                self.selected_strategy_id.as_deref()
                    == Some(id.as_str());
            content = content.push(self.strategy_list_row(
                id,
                strategy.as_ref(),
                is_selected,
            ));
    ...
    fn strategy_list_row(
        &self,
        id: &str,
        strategy: &dyn backtest::BacktestStrategy,
        is_selected: bool,
    ) -> Element<'_, Message> {
        ...
        let name_text =
            text(meta.name.clone()).size(tokens::text::BODY);
