...
use crate::modals::pane::indicator::helpers::{
    format_float, format_integer,
};
...
use iced::{
    ...
    widget::{
        button, center, column, container, pick_list, row, rule,
        scrollable,
...
impl BacktestLaunchModal {
    ...
    pub(super) fn view_right_panel(&self) -> Element<'_, Message> {
        let Some(ref selected_id) = self.selected_strategy_id
        else {
            return center(
        ...
        let Some((_, strategy)) = self
            ...
            return center(
                EmptyStateBuilder::new("Strategy not found")
                    .icon(Icon::Close),
            )
            .into();
        };
        ...
        let tab_items: Vec<(String, Message)> = SettingsTab::ALL
            .iter()
            .map(|tab| {
                (tab.to_string(), Message::TabChanged(*tab))
            })
            .collect();
        ...
        let tab_content: Element<'_, Message> =
            match self.settings_tab {
                SettingsTab::Parameters => {
                    self.view_parameters_tab(selected_id, strategy.as_ref())
                }
                SettingsTab::Dataset => self.view_dataset_tab(),
                SettingsTab::Execution => {
                    self.view_execution_tab()
                }
            };
    ...
    fn view_parameters_tab<'a>(
        &'a self,
        study_id: &str,
        strategy: &'a dyn backtest::BacktestStrategy,
    ) -> Element<'a, Message> {
        ...
        let visible: Vec<&study::ParameterDef> = params
            .iter()
            .filter(|p| {
                p.tab == study::ParameterTab::Parameters
            })
        ...
        let mut content_col: Vec<Element<'a, Message>> =
            Vec::new();
        ...
        for section_def in &section_order {
            ...
            let title =
                section_label.unwrap_or("Parameters");
    ...
    fn view_dataset_tab(&self) -> Element<'_, Message> {
        if self.available_tickers.is_empty() {
            return center(
                EmptyStateBuilder::new(
                    "No data available \u{2014} connect a data \
                     feed to run backtests",
                )
                .icon(Icon::Cog),
            )
            .into();
        ...
        // Ticker dropdown from DataIndex
        let display_names: Vec<String> = self
            ...
            .collect();
        let selected_display = self.selected_ticker.as_ref().and_then(
            |sel| {
        ...
        let ticker_dropdown = pick_list(
            ...
            move |selected: String| {
                // Extract symbol before the first space
                let sym = selected
                    .split(' ')
                    .next()
                    .unwrap_or(&selected);
        ...
        let timeframe_options: Vec<data::Timeframe> =
            data::Timeframe::KLINE.to_vec();
        let timeframe_strs: Vec<String> = timeframe_options
            .iter()
            .map(|tf| tf.to_string())
            .collect();
        ...
        let timeframe_dropdown = pick_list(
            ...
            move |selected: String| {
                ...
                Message::TimeframeSelected(
                    timeframe_options[idx],
                )
            },
        )
        .width(Length::Fill);

        let instrument_section =
        ...
        let calendar_view =
            self.calendar.view(Message::Calendar);

        let date_section =
            FormSectionBuilder::new("Date Range")
                .push(calendar_view);

        column![instrument_section, date_section]
    ...
    fn view_execution_tab(&self) -> Element<'_, Message> {
        // Capital section
        let capital_section =
            FormSectionBuilder::new("Capital")
                .push(form_row(
                    "Initial Capital ($)",
                    text_input("100000", &self.initial_capital_str)
                        .on_input(
                            Message::InitialCapitalChanged,
                        )
                        .width(Length::Fill),
                ))
                .push(form_row(
        ...
        let mut slippage_section =
            FormSectionBuilder::new("Slippage").push(form_row(
                "Mode",
                pick_list(
                    slippage_options,
                    selected_slippage,
        ...
        let sizing_section =
            FormSectionBuilder::new("Position Sizing")
                .push(form_row(
                    "Mode",
                    pick_list(
                        size_options,
        ...
        let mut risk_section =
            FormSectionBuilder::new("Risk Limits").push(
                crate::components::input::toggle_switch::toggle_switch(
                    "Max Drawdown Limit",
        ...
        // Session section
        let session_section =
            FormSectionBuilder::new("Session")
                .push(form_row(
                    "RTH Open (HHMM)",
