            ...
            .and_then(|pos| self.find_nearest_trade(pos, bounds))
        {
            return Some(
                canvas::Action::publish(
                    ManagerMessage::SelectTrade(Some(idx)),
                )
                .and_capture(),
            );
        }
...
impl<'a> EquityChart<'a> {
    fn chart_params(
        ...
        let min_ts =
            curve.points.first().map(|p| p.timestamp.0).unwrap_or(0);
        let max_ts =
            curve.points.last().map(|p| p.timestamp.0).unwrap_or(1);
    ...
    fn draw_base(&self, frame: &mut Frame, bounds: Rectangle) {
            ...
            if peak > point.total_equity_usd {
                ...
                if dd_h > 0.5 {
                    frame.fill_rectangle(
                        ...
                        Size::new(2.0, dd_h),
                        Fill {
                            style: tokens::backtest::DRAWDOWN_FILL
                                .into(),
                            ..Default::default()
                        },
                    );
            ...
            .and_then(|idx| self.result.trades.get(idx))
        {
            ...
            frame.fill_rectangle(
                ...
                Fill {
                    style: tokens::backtest::SELECTED_FILL
                        .into(),
                    ..Default::default()
                },
            );
    ...
    fn draw_overlay(
        ...
        let snap_y =
            Self::eq_to_y(&p, snap_point.total_equity_usd);
...
impl<'a> canvas::Program<ManagerMessage>
    for PropFirmEquityChart<'a>
{
    type State = ChartHoverState;
...
impl<'a> PropFirmEquityChart<'a> {
    fn chart_params(
        ...
        let target_eq = self.account_size
            * (1.0 + self.profit_target_pct / 100.0);
        let dd_limit_eq = self.account_size
            * (1.0 - self.max_drawdown_pct / 100.0);
    ...
    fn draw_base(&self, frame: &mut Frame, bounds: Rectangle) {
        ...
        let path = Path::new(|b| {
            ...
            for (i, &eq) in
                self.equity_curve.iter().enumerate().skip(1)
            {
                b.line_to(Point::new(
                    Self::idx_to_x(&p, i),
                    Self::eq_to_y(&p, eq),
        ...
        if let Some(idx) = self.breach_trade_idx {
            ...
            let by =
                Self::eq_to_y(&p, self.equity_curve[curve_idx]);
            let circle = Path::circle(Point::new(bx, by), 4.0);
            frame.fill(
                &circle,
                Fill {
                    style: tokens::backtest::PROP_FIRM_LIMIT
                        .into(),
                    ..Default::default()
                },
            );
    ...
    fn draw_overlay(
        ...
        let raw_idx = ((cursor.x - p.pad) / usable
            * max_idx as f32)
            .round() as usize;
