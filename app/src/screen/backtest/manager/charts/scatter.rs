...
impl<'a> canvas::Program<ManagerMessage> for ScatterChart<'a> {
    ...
    fn update(
            ...
            .and_then(|pos| self.find_nearest_point(pos, bounds))
        {
            ...
            return Some(
                canvas::Action::publish(ManagerMessage::SelectTrade(Some(trade_idx))).and_capture(),
            );
        }
    ...
    fn mouse_interaction(
        ...
        if cursor.is_over(bounds) {
            let has_nearby = state
                .cursor
                .is_some_and(|pos| self.find_nearest_point(pos, bounds).is_some());
            if has_nearby {
...
impl<'a> ScatterChart<'a> {
    fn scatter_params(&self, bounds: Rectangle) -> ScatterParams {
        let pad = 20.0_f32;
        let w = bounds.width;
        let h = bounds.height;

        let min_mae = self.points.iter().map(|(m, _, _, _)| *m).min().unwrap_or(0);
        let max_mae = self.points.iter().map(|(m, _, _, _)| *m).max().unwrap_or(1);
        let min_mfe = self.points.iter().map(|(_, m, _, _)| *m).min().unwrap_or(0);
        let max_mfe = self.points.iter().map(|(_, m, _, _)| *m).max().unwrap_or(1);
    ...
    fn mae_to_x(p: &ScatterParams, mae: i64) -> f32 {
        p.pad + ((mae - p.min_mae) as f64 / p.x_range * p.usable_w) as f32
    }

    fn mfe_to_y(p: &ScatterParams, mfe: i64) -> f32 {
        p.pad + ((p.max_mfe - mfe) as f64 / p.y_range * p.usable_h) as f32
    }

    fn find_nearest_point(&self, pos: Point, bounds: Rectangle) -> Option<usize> {
        ...
        for (i, &(mae, mfe, _, _)) in self.points.iter().enumerate() {
            ...
            let dist = ((x - pos.x).powi(2) + (y - pos.y).powi(2)).sqrt();
    ...
    fn draw_base(&self, frame: &mut Frame, bounds: Rectangle) {
        ...
        let mae_label = Text {
            ...
            position: Point::new(p.w * 0.5 - 30.0, p.h - p.pad * 0.5),
    ...
    fn draw_overlay(&self, frame: &mut Frame, cursor: Point, bounds: Rectangle) {
        ...
        if let Some(idx) = self.find_nearest_point(cursor, bounds) {
            let (mae, mfe, is_winner, _trade_idx) = self.points[idx];
            let x = Self::mae_to_x(&p, mae);
            let y = Self::mfe_to_y(&p, mfe);

            let highlight = Path::circle(Point::new(x, y), 7.0);
            frame.stroke(
                &highlight,
                Stroke {
