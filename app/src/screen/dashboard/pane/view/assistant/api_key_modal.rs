...
pub fn view_api_key_modal<'a>(
    ...
    let dismiss_msg = Message::PaneEvent(
        id,
        Event::AiAssistant(AiAssistantEvent::DismissApiKeyModal),
    );
    let save_msg = Message::PaneEvent(
        id,
        Event::AiAssistant(AiAssistantEvent::SaveApiKey),
    );
    let dismiss_msg2 = dismiss_msg.clone();
    let dismiss_msg3 = dismiss_msg.clone();

    let modal_content: Element<'a, Message> = container(
        column![
            row![
            ...
            column![
                ...
                small("Get a free API key at openrouter.ai/keys")
                    .style(palette::info_text),
                SecureFieldBuilder::new(
                    "API Key",
                    "sk-or-...",
                    &state.api_key_input,
                    move |s| Message::PaneEvent(
                        id,
                        Event::AiAssistant(
