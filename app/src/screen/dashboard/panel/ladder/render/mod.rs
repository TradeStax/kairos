...
impl canvas::Program<super::super::Message> for Ladder {
    type State = ();
    ...
    fn update(
        ...
        match event {
            Event::Mouse(mouse::Event::ButtonPressed(
                ...
                canvas::Action::publish(super::super::Message::ResetScroll).and_capture(),
            ),
            Event::Mouse(mouse::Event::WheelScrolled { delta }) => {
                ...
                Some(
                    canvas::Action::publish(super::super::Message::Scrolled(
                        scroll_amount,
                    ))
                    .and_capture(),
                )
                    ...
                    match visible_row.row {
                        ...
                        DomRow::Spread => {
                            if let Some(spread) = self.raw_price_spread {
                                let min_ticksize_f32 =
                                    self.ticker_info.min_ticksize.to_f32_lossy();
                                let min_ticksize =
                                    exchange::util::MinTicksize::from(min_ticksize_f32);
                                spread_row =
                                    Some((visible_row.y, visible_row.y + ROW_HEIGHT));

                                let spread =
                                    spread.round_to_min_tick(min_ticksize);
                                let content = format!(
                                    "Spread: {}",
                                    spread.fmt_with_precision(min_ticksize)
                                );
