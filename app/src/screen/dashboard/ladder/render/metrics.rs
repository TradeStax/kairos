...
impl Ladder {
    ...
    pub(super) fn draw_chase_trail(
        ...
        if let Some((start_p_units, end_p_units, alpha)) = tracker.segment() {
            let start_p =
                Price::from_units(start_p_units).round_to_side_step(is_bid, grid.tick.into());
            let end_p = Price::from_units(end_p_units).round_to_side_step(is_bid, grid.tick.into());
    ...
    pub(super) fn build_price_grid(&self) -> Option<PriceGrid> {
        let best_bid = match (
            self.best_price(data::Side::Bid),
            self.best_price(data::Side::Ask),
        ) {
            (Some(bb), _) => bb,
            (None, Some(ba)) => ba.add_steps(-1, self.tick_size.into()),
            (None, None) => {
                let (min_t, max_t) = self.trades.price_range()?;
                let steps = Price::steps_between_inclusive(min_t, max_t, self.tick_size.into())
                    .unwrap_or(1);
    ...
    pub(super) fn price_to_screen_y(
        ...
        let idx = if price >= grid.best_ask {
            let steps = Price::steps_between_inclusive(grid.best_ask, price, grid.tick.into())?;
            -(steps as i32)
        } else if price <= grid.best_bid {
            let steps = Price::steps_between_inclusive(price, grid.best_bid, grid.tick.into())?;
            steps as i32
        } else {
