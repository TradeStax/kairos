...
pub fn render_sparse_trades(
    ...
    for (time, dp) in heatmap_data.trades_by_time.range(earliest..=latest) {
        ...
        for (price, qty, is_sell) in dp.iter_trades() {
            ...
            if qty > trade_size_filter {
                ...
                let radius = {
                    if let Some(scale) = trade_size_scale {
                        ...
                        1.0 + (qty / max_trade_qty) * (MAX_CIRCLE_RADIUS - 1.0) * scale_factor
                    } else {
                        cell_height / 2.0
                    }
                };
...
pub fn render_dense_trades(
    ...
    for (time, dp) in heatmap_data.trades_by_time.range(earliest..=latest) {
        ...
        for (price, qty, is_sell) in dp.iter_trades() {
            ...
            let trade_price_units = price.units();
            ...
            let alpha = (0.3 + (qty / max_trade_qty) * 0.7).min(1.0);
