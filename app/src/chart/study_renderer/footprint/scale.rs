...
use data::ChartBasis;
use study::output::{
    FootprintCandle, FootprintData, FootprintDataType, FootprintLevel, FootprintScaling, TextFormat,
};
...
/// coarser grouping.
pub(super) fn compute_dynamic_quantum(state: &ViewState, factor: i64, tick_units: i64) -> i64 {
    super::super::coord::compute_dynamic_quantum(state, MIN_ROW_PX, factor, tick_units)
}
...
pub(super) fn merge_levels_to_quantum(
    ...
    for level in levels {
        let rounded = (level.price / target_quantum) * target_quantum;
...
pub(super) fn format_value(value: f32, format: TextFormat) -> String {
    match format {
...
pub(super) fn effective_cluster_qty(
    ...
    let individual_max = match data_type {
        FootprintDataType::BidAskSplit | FootprintDataType::DeltaAndVolume => levels
            .iter()
    ...
    match scaling {
        ...
        FootprintScaling::Linear | FootprintScaling::Sqrt | FootprintScaling::Log => {
            safe(visible_max)
        }
    }
}
...
#[inline]
pub(super) fn scaled_ratio(qty: f32, max: f32, scaling: FootprintScaling) -> f32 {
    if max <= f32::EPSILON || qty <= f32::EPSILON {
        return 0.0;
    }
    match scaling {
        FootprintScaling::Sqrt => qty.sqrt() / max.sqrt(),
        FootprintScaling::Log => (1.0 + qty).ln() / (1.0 + max).ln(),
        _ => qty / max,
    }
}
...
pub(super) fn calc_visible_max(
    ...
    let candles_iter: Box<dyn Iterator<Item = &FootprintCandle>> = match basis {
        ChartBasis::Time(_) => Box::new(
            data.candles
                .iter()
                .filter(move |c| c.x >= earliest && c.x <= latest),
        ),
        ChartBasis::Tick(_) => {
    ...
    candles_iter
        .map(|c| match dynamic_quantum {
            Some(q) if q > c.quantum => {
                let merged = merge_levels_to_quantum(&c.levels, q);
                level_max(&merged.0)
            }
