...
//! Pure data types without any rendering or interaction logic.
...
pub enum DrawingTool {
    ...
    VolumeProfile,
    /// Delta profile (VBP with delta mode, 2 points define time range)
    DeltaProfile,
...
impl DrawingTool {
    /// Whether this tool is a VBP-style drawing (VolumeProfile or DeltaProfile).
    pub fn is_vbp(self) -> bool {
        matches!(self, DrawingTool::VolumeProfile | DrawingTool::DeltaProfile)
    }
...
impl std::fmt::Display for DrawingTool {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            DrawingTool::DeltaProfile => write!(f, "Delta Profile"),
...
pub struct DrawingStyle {
    ...
    /// VBP drawing config (for VolumeProfile/DeltaProfile tools)
    #[serde(default)]
